...
def generate_configs():
    output_dir = "configs/aloi_ablation_z8_z4"
    os.makedirs(output_dir, exist_ok=True)
    
    # Common settings (Same as previous ALOI experiments)
    common_config = {
        "experiment": {
            "dataset": "aloi",
            "device": "auto",
    ...
    # Experiments definition: Z=8 and Z=4
    experiments = []
    
    # Z = 8
    experiments.append({"z": 8, "mode": "direct", "enc": "mlp", "ecc": "none", "L": 1})
    for L in [2, 4, 8, 16]:
        for ecc in ["random_gaussian", "repetition"]:
            experiments.append({"z": 8, "mode": "semi_amortized", "enc": "cnn2d", "ecc": ecc, "L": L})
            
    # Z = 4
    experiments.append({"z": 4, "mode": "direct", "enc": "mlp", "ecc": "none", "L": 1})
    for L in [2, 4, 8, 16]:
        for ecc in ["random_gaussian", "repetition"]:
            experiments.append({"z": 4, "mode": "semi_amortized", "enc": "cnn2d", "ecc": ecc, "L": L})
